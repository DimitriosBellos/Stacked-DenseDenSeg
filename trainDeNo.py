...
def train_net(net, sm, options):
    ...
    criterion = nn.MSELoss(reduce=options.reduce)
    ...
    for epoch in range(0, epochs):
        ...
        for instance in range(0,instances):
            ...
            if options.gpu:
                acc = loss.cpu().detach().numpy()
                tot += acc
            else:
                acc = loss.detach().numpy()
                tot += acc
        ...
        acc = eval2_net(net, options, data, ['Validation', 'val'])
    ...
    acc = eval2_net(net, options, data, ['Testing','test'])
    time = timer.get_value()
