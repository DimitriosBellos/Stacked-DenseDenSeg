...
def img(itr):
    j=int(itr//21)
    i=int(itr-j*21)
    f2=h5py.File('/db/psxdb3/4D/4D_%d.h5' % itr, 'r')
    inp=f2['data'][1205, 806:1414, 50:562]
    f2.close()
    f2=h5py.File('/db/psxdb3/4D/4D_%d_infer/Pred_%d_den.h5' % (itr, itr), 'r')
    inp2=f2['data'][1205, 806:1414, 50:562]
    f2.close()
    f2=h5py.File('/db/psxdb3/4D/original.h5', 'r')
    data0=f2['data'][itr, 1205, 806:1414, 50:562] #806+40:806+110, 50+190:50+310]
    f2.close()
    f2=h5py.File('/db/psxdb3/4D/hmm1_5.h5', 'r')
    data=f2['data'][itr, 1205, 806:1414, 50:562]
    f2.close()
    f2=h5py.File('/db/psxdb3/4D/hmm2_5.h5', 'r')
    data2=f2['data'][itr, 1205, 806:1414, 50:562]
    f2.close()
    # f2 = h5py.File('/db/psxdb3/4D/lq_annotations.h5', 'r')
    ...
    name='/home/psxdb3/Code_Folder/4D_results/Clear/ImageClear_%d' % itr
