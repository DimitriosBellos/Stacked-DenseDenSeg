...
write_filename='/home/psxdb3/Code_Folder/4D_results/High_trans/Voxels'
write_filename_im='/home/psxdb3/Code_Folder/4D_results/High_trans/Or/'
write_filename2='/db/psxdb3/4D/original.h5'
time_size=21
n_images=64
image_line_size=31
...
def img(idx, itr):
    ...
    for t in range(0, time_size):
        location = '/db/psxdb3/4D/4D_%d.h5' % t
        f2 = h5py.File(location, 'r')
        data = f2.get(list(f2.keys())[0].encode('ascii', 'ignore'))
        while isinstance(data, h5py.Group):
            data = data.get(list(data.keys())[0].encode('ascii', 'ignore'))
        ...
        f2.close()
        location = '/db/psxdb3/4D/4D_%d_infer/Pred_%d_den.h5' % (t, t)
        f2 = h5py.File(location, 'r')
        data = f2.get(list(f2.keys())[0].encode('ascii', 'ignore'))
        while isinstance(data, h5py.Group):
            data = data.get(list(data.keys())[0].encode('ascii', 'ignore'))
        ...
        f2.close()
        location = '/db/psxdb3/4D/4D_%d_infer/Pred_%d.h5' % (t, t)
        f2 = h5py.File(location, 'r')
        data = f2.get(list(f2.keys())[0].encode('ascii', 'ignore'))
        while isinstance(data, h5py.Group):
            data = data.get(list(data.keys())[0].encode('ascii', 'ignore'))
