...
def Options():
    ...
    parser.add_option('--gpu-devices', dest='gpu_devices', default='2,3', type='string', help='Select CUDA_VISIBLE_DEVICES (default:0)')
    # ----------------------------------------------Model options----------------------------------------------------------
    parser.add_option('--num-classes', dest='n_classes', default=11, type='int', help='Set Number of Classes (default:3)')
    ...
    # ---------------------------------------------Training options--------------------------------------------------------
    parser.add_option('--epochs', dest='epochs', default=150, type='int', help='Number of Epochs (default:5000)')
    ...
    parser.add_option('--batch-size', dest='batchsize', default=4, type='int', help='Batch Size (default:8)')
    ...
    parser.add_option('--load', dest='load', default='0', type='string', help='Load Saved Model')  # /db3/psxdb3/SemSegCT/Checkpoints/CP12.pth
    parser.add_option('--weighted', action='store_true', dest='weighted', default=False, help='Use weights in criterion (True/False)')
    parser.add_option('--reduce', action='store_false', dest='reduce', default=True, help='Use reduce dimensions in criterion (True/False)')
    # -----------------------------------------------Data options----------------------------------------------------------
    # parser.add_option('--input-filename', dest='input_filename', default='/db3/psxdb3/DataVol/channels/0_thresh.h5', type='string', help='Select Input Filename')
    parser.add_option('--input-filename', dest='input_filename', default='/db3/psxdb3/CamVid', type='string', help='Select Input Filename')
    ...
    parser.add_option('--input-size', dest='input_size', default='3,360,480', type='string', help='Input Size (default: 5,512,512 )')
    ...
    parser.add_option('--output-area', dest='output_area', default='0,0,0,1,360,480', type='string', help='Output Area in relation with the Input (default: 2,0,0,1,512,512 )')
    parser.add_option('--input-channels', dest='input_channels', default=1, type='int', help='Input Channels (default:1)')
    parser.add_option('--input-stride', dest='input_stride', default='1,512,512', type='string', help='Input stride (default:1,512,512)')
    ...
    parser.add_option('--mask-class', dest='mask_class', default=11, type='int', help='Use normalize inputs (True/False)')
    parser.add_option('--rot_deg', dest='rotdeg', default=0, type='int', help='Use normalize inputs (True/False)')
    # --------------------------------------------Validation options-------------------------------------------------------
    parser.add_option('--val-perc', dest='val_precentage', default=0.1, type='float', help='Validation Precentage (default:0.1)')
    parser.add_option('--test-perc', dest='test_precentage', default=0.1, type='float', help='Testing Precentage (default:0.1)')
    parser.add_option('--val-freq', dest='val_freq', default=1, type='int', help='Validation Frequency every n epochs (default:1)')
    parser.add_option('--cp-dest', dest='cp_dest', default='Checkpoints/', type='string', help='Checkpoints Save Directory (default:0.1)')
    return parser
