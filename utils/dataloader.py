...
class DataloaderMSDold(object):
    def __init__(self, options):
        super(DataloaderMSDold, self).__init__()
        ...
        if isfile(options.cp_dest + 'dataloader.npz') and options.clean_go is False:
            files = np.load(options.cp_dest + 'dataloader.npz')
            order = files['order']
            perm = files['perm']
            rotstate = files['rotstate']
            weights = files['weights']
        ...
        permTrain = perm[0:num_train]
        permVal = perm[num_train:(num_train + num_val)]
        permTest = perm[(num_train + num_val):len(perm)]
        ...
        for i in range(0, num_train):
            ...
            self.batchTrain[batch, instance, 2] = order[permTrain[i], 2]
            self.batchTrain[batch, instance, 1] = order[permTrain[i], 1]
            self.batchTrain[batch, instance, 0] = order[permTrain[i], 0]
        for i in range(0, num_val):
            instance = np.mod(i - num_train, options.batchsize).astype(int)
            batch = i // options.batchsize
            self.batchVal[batch, instance, 3] = rotstate[i + num_train]
            self.batchVal[batch, instance, 2] = order[permVal[i], 2]
            self.batchVal[batch, instance, 1] = order[permVal[i], 1]
            self.batchVal[batch, instance, 0] = order[permVal[i], 0]
        for i in range(0, num_test):
            ...
            self.batchTest[batch, instance, 2] = order[permTest[i], 2]
            self.batchTest[batch, instance, 1] = order[permTest[i], 1]
            self.batchTest[batch, instance, 0] = order[permTest[i], 0]
        np.savez(options.cp_dest + 'dataloader.npz', order=order, perm=perm, rotstate=rotstate, weights=weights)
    ...
    def preprocess(self, options):
        ...
        weights = np.zeros((options.n_classes,))
        voxcount = 0
        for i in range(0, self.numel[0]):
            ...
            for j in range(0, self.numel[1]):
                for k in range(0, self.numel[2]):
                    ...
                    # out = False
                    ex_data = torch.from_numpy(DataloaderMSDold.getWindow(self.dataA,
                    ...
                    if ex_data.max() > 0:  # other class present except Air: 0
                        ...
                        voxcount += ex_data.numel()
                        for r in range(0, options.n_classes):
                            weights[r] += (ex_data == r).sum()
                    else:
                        if np.random.uniform(0, 100,
                                             1) < 15:  # Add Air wot 50% chance provided the 512x512 lays completely in the data
                            order = np.vstack([order, [z, y, x]])
                            weights[0] += ex_data.numel()
                            voxcount += ex_data.numel()
                    DataloaderMSDold.progress(count, total, status='Data Preprocessing')
        sys.stdout.write("\n")
        weights = weights / voxcount
        weights = (options.n_classes + 1) / options.n_classes - weights
    ...
    def load(self, epoch, options, mode):
        ...
        torch_dataA = torch.empty(batchsize,
                                  int(options.output_area[1, 1]),
                                  int(options.output_area[1, 2]))
        for i in range(0, batchsize):
            ...
            if options.normalize:
                torch_dataI[i, :, :, :] = DataloaderMSDold.normalize(
                    torch.from_numpy(DataloaderMSDold.rotate(DataloaderMSDold.getWindow(self.dataI,
                                                                                        z,
                                                                                        y,
                                                                                        x,
                ...
                torch_dataI[i, :, :, :] = torch.from_numpy(
                    DataloaderMSDold.rotate(DataloaderMSDold.getWindow(self.dataI,
                                                                       z,
                                                                       y,
                                                                       x,
                                                                       options.input_size[0],
                                                                       ...
                                                                       options.input_size[2],
                                                                       self.pad), rtst).copy())
            # self.dataI[z:z+options.input_size[0], y:y+options.input_size[1], x:x+options.input_size[2]])
            torch_dataA[i, :, :] = torch.from_numpy(DataloaderMSDold.rotate(DataloaderMSDold.getWindow(self.dataA,
                                                                                                       z +
                                                                                                       options.output_area[
                                                                                                           0, 0],
                                                                                                       y +
    ...
    def load3D(self, epoch, options, mode):
        # Returns a batch for each epoch
        # Edited
        if mode == 'train':
            data = self.batchTrain
        ...
        for i in range(0, batchsize):
            ...
            x = data[epoch, i, 2]
            rtst = int(data[epoch, i, 3])
            torch_dataI[i, :, :, :, :] = torch.from_numpy(DataloaderMSDold.rotate(DataloaderMSDold.getWindow(self.dataI,
                                                                                                             z,
                                                                                                             y,
                                                                                                             x,
                                                                                                             options.input_size[
