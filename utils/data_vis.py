    ...
    if options.dice == 'MSE':
        if options.data_3D is False:
            ...
            if x.shape[0] != 1:
                ...
                # axarr[1, i].set_title("annotations")
                axarr[2].axis('off')
    elif options.CamVid is True:
        x = x.permute(0, 2, 3, 1)
        for i in range(0, x.shape[0]):
            x[i, :, :, :] = (x[i, :, :, :] - x[i, :, :, :].min()) / (x[i, :, :, :].max() - x[i, :, :, :].min())
        if x.shape[0] != 1:
            for i in range(0, x.shape[0]):
                axarr[0, i].imshow(x[i, :, :, :].detach().numpy())
                # axarr[0, i].set_title("input")
                axarr[0, i].axis('off')

                axarr[1, i].imshow(y[i, :, :].detach().numpy(), vmin=0, vmax=options.n_classes - 1, cmap='Set3')
                # axarr[1, i].set_title("annotations")
                axarr[1, i].axis('off')

                axarr[2, i].imshow(y_pred[i, :, :].detach().numpy(), vmin=0, vmax=options.n_classes - 1, cmap='Set3')
                # axarr[1, i].set_title("annotations")
                axarr[2, i].axis('off')
        else:
            axarr[0].imshow(x[0, :, :, :].detach().numpy())
            # axarr[0, i].set_title("input")
            axarr[0].axis('off')

            axarr[1].imshow(y[0, :, :].detach().numpy(), vmin=0, vmax=options.n_classes, cmap='Set3')
            # axarr[1, i].set_title("annotations")
            axarr[1].axis('off')

            axarr[2].imshow(y_pred[0, :, :].detach().numpy(), vmin=0, vmax=options.n_classes - 1, cmap='Set3')
            # axarr[1, i].set_title("annotations")
            axarr[2].axis('off')
    else:
        if options.data_3D is False:
