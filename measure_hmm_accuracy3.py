...
class Databringer(Dataset):
    def __init__(self, n_classes, time_size, slices, ground_truths, before_hmm, after_hmm, after_hmm2, start, divisions, cpus):
        ...
        self.b = self.slices
        self.b[:, 0] = self.b[:, 0] / self.divisions[0]
        self.b[:, 1] = self.b[:, 1] / self.divisions[1]
        self.b[:, 2] = self.b[:, 2] / self.divisions[2]
    ...
    def __getitem__(self, idx):
        ...
        z = idx // (self.divisions[1] * self.divisions[2])
        tr = idx - z * (self.divisions[1] * self.divisions[2])
        y = tr // self.divisions[2]
        x = tr - y * self.divisions[2]
        ...
        tmp = data[((z * self.b[self.iteration, 0]) + self.start[self.iteration, 0]):(((z+1) * self.b[self.iteration, 0]) + self.start[self.iteration, 0]),
                   ((y * self.b[self.iteration, 1]) + self.start[self.iteration, 1] + 450):(((y+1) * self.b[self.iteration, 1]) + self.start[self.iteration, 1] + 450),
                   ((x * self.b[self.iteration, 2]) + self.start[self.iteration, 2] + 401):(((x+1) * self.b[self.iteration, 2]) + self.start[self.iteration, 2] + 401)]
        tmp[tmp == 4] = 3
        ...
        while isinstance(data, h5py.Group):
            data = data.get(list(data.keys())[0].encode('ascii', 'ignore'))
        Samy = data[:,
                    ((z * self.b[self.iteration, 0]) + self.start[self.iteration, 0]):(((z + 1) * self.b[self.iteration, 0]) + self.start[self.iteration, 0]),
                    ((y * self.b[self.iteration, 1]) + self.start[self.iteration, 1]):(((y + 1) * self.b[self.iteration, 1]) + self.start[self.iteration, 1]),
                    ((x * self.b[self.iteration, 2]) + self.start[self.iteration, 2]):(((x + 1) * self.b[self.iteration, 2]) + self.start[self.iteration, 2])]
        f2.close()
        Samy = Samy.flatten()
...
if __name__ == '__main__':
    ground_truths = '/db/psxdb3/havok/db1/annotations/annotations0.h5'
    before_hmm = '/db/psxdb3/4D/original.h5'
    after_hmm = '/db/psxdb3/4D/hmm1_5.h5'
    after_hmm2 = '/db/psxdb3/4D/hmm2_5.h5'
    time_size = 21
    ...
    val = LoggerLite('/home/psxdb3/Code_Folder/validationshmm_new2_5', 'w')
    t = ('Methods',)
    ...
    cms = LoggerLite('/home/psxdb3/Code_Folder/cmshmm_new2_5', 'w')
    t = ('Methods',)
    ...
    data = Databringer(n_classes, time_size, slices, ground_truths, before_hmm, after_hmm, after_hmm2, start, divisions, cpus)
